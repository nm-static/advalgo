...
description: "1. Storing Files on a Tape - Advanced Algorithms"
section: main
sidebar:
  order: 1
  label: "1. Storing Files on a Tape"
---
...
Suppose we have a set of $n$ files that we want to store on magnetic tape<label for="sn-1" class="sidenote-number"></label><input type="checkbox" id="sn-1" class="sidenote-toggle"/><span class="sidenote">Magnetic tape storage was once the primary medium for data backup and archival. While largely replaced by disk and cloud storage, tapes are still used in some enterprise settings for cold storage due to their durability and cost-effectiveness.</span>. In the future, users will want to read those files from the tape. Reading a file from tape isn't like reading a file from disk; first we have to fast-forward past all the other files, and that takes a significant amount of time. Let $L[1 .. n]$ be an array listing the lengths of each file; specifically, file $i$ has length $L[i]$. If the files are stored in order from 1 to $n$, then the cost of accessing the $k$th file is
...
<div class="interactive-embed" data-interactive-id="tape-sort"></div>

### Greedy Works Out

<details class="callout callout-tip">
<summary class="callout-title">Correctness of Greedy - I</summary>
<div class="callout-body">
...
$$\Sigma \operatorname{cost}(\pi)=\sum_{k=1}^{n}\left(F[\pi(k)] \cdot \sum_{i=1}^{k} L[\pi(i)]\right)=\sum_{k=1}^{n} \sum_{i=1}^{k}(F[\pi(k)] \cdot L[\pi(i)]).$$
...
<div class="callout-title">Food For Thought</div>
<div class="callout-body">
...
<details class="callout callout-tip">
<summary class="callout-title">Correctness of Greedy - II</summary>
<div class="callout-body">
...
[^1]: Just in case you have not heard of them, here's [Wikipedia on magnetic tapes](https://en.wikipedia.org/wiki/Magnetic_tape) :)
